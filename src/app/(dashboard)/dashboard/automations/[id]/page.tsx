...
import {
  ...
  Star, EyeOff, Share2, Smartphone, BarChart2, Eye, Heart, ScrollText,
} from "lucide-react";
...
export default function AutomationDetailPage() {
  ...
  const fetchAutomationLogDates = useCallback(async () => {
    try {
      const res = await fetch(`/api/scheduler/logs?automationId=${id}`, { cache: "no-store" });
      const json = await res.json();
      const dates = Array.isArray(json.dates) ? (json.dates as string[]) : [];
      setLogDates(dates);
      if (dates.length > 0 && !selectedLogDate) setSelectedLogDate(dates[0]);
    } catch {
      setLogDates([]);
    }
  }, [id, selectedLogDate]);

  useEffect(() => { fetchAutomationLogDates(); }, [fetchAutomationLogDates]);

  const fetchAutomationLogContent = useCallback(async (date: string) => {
    if (!date) return;
    setLoadingLogs(true);
    try {
      const res = await fetch(`/api/scheduler/logs?automationId=${id}&date=${date}`, { cache: "no-store" });
      const json = await res.json();
      setLogContent(typeof json.content === "string" ? json.content : "");
    } catch {
      setLogContent("");
    } finally {
  ...
  useEffect(() => {
    ...
    fetchAutomationLogContent(selectedLogDate);
  }, [selectedLogDate, fetchAutomationLogContent]);
      ...
      {videos.length === 0 ? (
        ...
        <div className="space-y-3">
          {videos.map((video) => {
            ...
            const triggerCtx = video.sourceMetadata?.generationContext;
            return (
                    ...
                    <Link href={`/dashboard/videos/${video.id}`} className="flex-1 min-w-0">
                      ...
                      {triggerCtx && (
                        <p className="text-[11px] text-muted-foreground mt-1">
                          Generated by <span className="font-medium text-foreground">{triggerCtx.triggerLabel || triggerCtx.triggerSource || "unknown"}</span>
                          {triggerCtx.reason ? ` · ${triggerCtx.reason}` : ""}
                        </p>
                      )}
                    </Link>
      ...
      <div className="space-y-4">
              ...
              <div className="space-y-1.5 max-h-56 overflow-y-auto">
                {auto.schedulerLogs.map((entry) => (
                  <div key={entry.id} className="text-xs rounded border p-2">
                    ...
                    {entry.videoId && (
                      <Link href={`/dashboard/videos/${entry.videoId}`} className="text-[11px] text-blue-600 hover:underline mt-1 inline-block">
                        View video {entry.videoId}
                      </Link>
                    )}
                  </div>
                ))}
        ...
        <Card>
          <CardContent className="p-4 space-y-3">
            <div className="flex items-center justify-between gap-2">
              <p className="text-xs font-medium text-muted-foreground">Worker / Scheduler / Poster logs</p>
              <select
              ...
              >
                {logDates.length === 0 ? (
                  ...
                  logDates.map((date) => <option key={date} value={date}>{date}</option>)
                )}
              </select>
            </div>
            <pre className="text-xs whitespace-pre-wrap rounded border bg-muted/20 p-3 max-h-96 overflow-auto">
              {loadingLogs ? "Loading logs..." : (logContent || "No logs for selected date.")}
            </pre>
          </CardContent>
        </Card>
      </div>
