...
const worker = new Worker<VideoJobData>(
  ...
  async (job) => {
    ...
    if (imageProvider === "FLOW_TV" || imageToVideoProvider === "FLOW_TV") {
      // Flow TV is now driven entirely by the run-machine
      // (src/services/flow-tv-run.ts) and finalized into a Video by
      // src/services/flow-tv-finalize.ts. The standard video-generation
      // worker no longer participates.
      throw new Error(
        "FLOW_TV is no longer routed through video-generation; use the Flow TV run-machine instead.",
      );
    }
    ...
    if (!scenes?.length && !completed.has("SCRIPT")) {
      ...
      const script = await generateScript(scriptInput, llmProvider, characterPrompt);
      title = script.title;
