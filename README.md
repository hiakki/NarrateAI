...
- **Setup everything and verify:** `scripts\setup_prerequisites.bat` (or `./scripts/setup_prerequisites.sh`)  
  Installs tools, starts Docker (Postgres + Redis), builds the app, and pushes the DB schema. Optionally use `--restore backups\your-backup.tar.gz` to restore a backup during setup.
