...
**Option A — Script (Windows: `.bat`, macOS/Linux: `.sh`)**

- **Setup everything and verify:** `scripts\setup_prerequisites.bat` (or `./scripts/setup_prerequisites.sh`)  
  Installs tools (Node 22, Docker, pnpm, FFmpeg, PM2, **cloudflared** for tunneling), starts Docker (Postgres + Redis), and pushes the DB schema. Optionally use `--restore backups\your-backup.tar.gz` to restore a backup during setup. **Localtunnel is not used**; only Cloudflare Tunnel (cloudflared) is supported.
- **Backup / restore only:** `scripts\backup-restore.bat backup` or `scripts\backup-restore.bat restore file.tar.gz`  
  Take a full backup or restore from one. Use `list` to see backups.
...
This starts the Next.js app, the video-generation worker, and the scheduler in one go. For production you can use PM2 (generated by the setup script).

**Network binding:** Next.js (`pnpm dev` / `pnpm start`) listens on **127.0.0.1** only (not `0.0.0.0`), so put **nginx** (or another reverse proxy) on **443** and proxy to `http://127.0.0.1:3000`. Docker Compose maps Postgres and Redis to **127.0.0.1** on the host as well. For a rare case where you need LAN access to the dev server, run `pnpm exec next dev -H 0.0.0.0` manually.
...
`pnpm dev:all` already runs the app, worker, and scheduler together. To run them in separate terminals:
